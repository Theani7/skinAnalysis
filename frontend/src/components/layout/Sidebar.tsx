...
import { LayoutDashboard, ScanLine, History, X, Sparkles, ChevronLeft, ChevronRight, Heart, ListChecks, Bot, Droplets } from 'lucide-react';
  ...
  const navGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'lifestyle', label: 'Lifestyle Log', icon: Droplets },
        ...
        <div className={`px-4 mt-6 mb-2 ${isCollapsed ? 'px-2' : ''}`}>
          <div className="relative group">
            <button
              ...
              className={`
                w-full flex items-center justify-center rounded-xl bg-primary-600 hover:bg-primary-700 text-white shadow-md shadow-primary-200 transition-all active:scale-[0.98]
                ${isCollapsed ? 'p-3' : 'gap-2 py-3 px-4'}
              `}
            >
              <ScanLine className="w-5 h-5 shrink-0" />
              {(!isCollapsed || sidebarOpen) && <span className="font-semibold text-sm lg:whitespace-nowrap">Scan Now</span>}
            </button>
            {isCollapsed && (
              <div className="hidden lg:group-hover:block absolute left-full top-1/2 -translate-y-1/2 ml-3 z-50 pointer-events-none">
                <div className="bg-primary-600 text-white text-xs font-medium px-3 py-2 rounded-lg whitespace-nowrap shadow-xl">
                  Scan Now
                </div>
              </div>
            )}
